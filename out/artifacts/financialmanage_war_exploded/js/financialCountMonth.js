...
//		alert("执行");
		...
		$.ajax({
			url : "/" + window.document.location.pathname.split("/")[1] + "/shouzhiRecord/MonthInDayCount.action",
			type : "get",
...
//					alert("incomeAll:"+incomeAll);
	...
	function MonthInCategoryCount(currentTime){
		var uid=$("#uid").val();
		$.ajax({
			url : "/" + window.document.location.pathname.split("/")[1] + "/shouzhiRecord/monthInCategoryCountIncome.action",
			type : "get",
			data : "currentTime=" + currentTime+"&uid="+uid,
			dataType : "json",// 返回时的数据类型json
			success : function(data) {
	...
	function MonthInCategoryCountSpends(currentTime){
		var uid=$("#uid").val();
		$.ajax({
			url : "/" + window.document.location.pathname.split("/")[1] + "/shouzhiRecord/monthInCategoryCountSpend.action",
			type : "get",
			data : "currentTime=" + currentTime+"&uid="+uid,
			dataType : "json",// 返回时的数据类型json
			success : function(data) {
