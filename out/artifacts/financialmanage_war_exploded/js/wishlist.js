...
//		var text=$("#add_msg").text();//提醒
		...
		$("#add_msg").text("");
		//符合校验，进行表单提交
		$.post("/" + window.document.location.pathname.split("/")[1] + "/wishlist/addWish.action", $("#addform").serialize(), function(data) { //序列化数据为对象
...
/*	//修改的值
	...
	$("#editButton").click(function(){
		...
		$.post("/" + window.document.location.pathname.split("/")[1] + "/wishlist/editWish.action", $("#editform").serialize(), function(data) { //序列化数据为对象
			//回调为ok时，弹出alert框，并重新刷新页面
			alert("修改心愿单成功！");
			window.location.reload();
		});
