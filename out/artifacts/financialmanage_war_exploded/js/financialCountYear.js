...
/*--------------------------------------------------------------------------------------------*/	
	...
	function yearInMonthCount(year) {
		...
		var uid=$("#uid").val();
		$.ajax({
			url : "/" + window.document.location.pathname.split("/")[1] + "/shouzhiRecord/yearInMonthCount.action",
			type : "get",
...
//		alert("饼图："+year);
		$.ajax({
				url : "/" + window.document.location.pathname.split("/")[1] + "/shouzhiRecord/yearInCategoryCount.action",
				type : "get",
				data : "year=" + year+"&uid="+uid,
//				data : "year=" + year,
				dataType : "json",// 返回时的数据类型json
...
//					 alert("success");
	...
	function yearInCategoryCountSpends(year) {
		var uid=$("#uid").val();
		$.ajax({
				url : "/" + window.document.location.pathname.split("/")[1] + "/shouzhiRecord/yearInCategoryCountSpends.action",
				type : "get",
				data : "year=" + year+"&uid="+uid,
//				data : "year=" + year,
				dataType : "json",// 返回时的数据类型json
