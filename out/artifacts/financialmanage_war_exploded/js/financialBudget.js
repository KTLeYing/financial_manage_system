$(function() {
	...
	$(window).load(function() {
		...
		functionAll1(uid, current, budgetValue);  //计算收入
...
//		alert(wnum);
		if(wnum==""||wnum==null){
			...
			$("#addmsg").text("");
			//添加预算
			 $.post("/" + window.document.location.pathname.split("/")[1] + "/budget/addBudget.action",$("#add_form").serialize(),function(data){
					alert("添加预算成功！");
	...
	$("#edit_btn").click(function(){
		...
		if(wnum==""||wnum==null){
			...
			$("#editmsg").text("");
			//添加预算
			 $.post("/" + window.document.location.pathname.split("/")[1] + "/budget/editBudget.action",$("#edit_form").serialize(),function(data){
					alert("编辑预算成功！");
					window.location.reload();
			});
		}
	...
	$("#delete_budget").click(function(){
		...
		if (confirm('确认要删除该条记录吗?')) {
			$.post("/" + window.document.location.pathname.split("/")[1] + "/budget/deleteBudget.action", {
				"wid" : wid
			}, function(data) {
	...
	function functionAll(uid,current,budgetValue){
		$.ajax({
			url : "/" + window.document.location.pathname.split("/")[1] + "/shouzhiRecord/monthInCategoryCount.action",
			type : "get",
			...
			success : function(data) {
				...
				console.log("data.spends:",data.spends);
				console.log("spends支出的长度：", data.spends.length);
				console.log("data.spends.length:" , data.spends.length);
