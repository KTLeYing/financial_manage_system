...
//			day=31;
			...
			var day2 = howDay(time2);
			var day2=1;
			currentTimeDay=currentTime+"-"+day;//当前月的最后一天
			// lastTimeDay=lastTime+"-"+day2;  //上个月的最后一天
...
//		$("#bottom_right").empty();
	...
	function AllMonthCount(currentTime,lastTime){//当前月  以及上月的收支情况统计分析
		var uid=$("#uid").val();
		$.ajax({
			url : "/" + window.document.location.pathname.split("/")[1] + "/financialAnalysis/monthAnalysis.action",
			type : "get",
...
//		alert("-----------------");
		$.ajax({
			url : "/" + window.document.location.pathname.split("/")[1] + "/financialAnalysis/monthCurrentDayAnalysis.action",
			type : "get",
...
//				alert("success");
				...
				console.log("current.incomes:", data.current.incomes);
				console.log("current.spends:", data.current.spends);
				console.log("last.incomes:", data.last.incomes);
				console.log("last.spends:", data.last.spends);
				...
				for(var i=0;i<data.current.spends.length;i++){//当前月支出
					...
					console.log("current.spends[i]:", data.current.spends[i].moneyName);
				}
				
				for(var i=0;i<data.last.spends.length;i++){//上个月支出
					lastAll+=data.last.spends[i].moneyName;
					console.log("last.spends[i]:", data.last.spends[i].moneyName);
				}
				if(-curAll>-lastAll){
					curPer=100+"%";
...
//				alert(curAll+":::"+lastAll+"-------"+curPer+":"+lastPer);
				...
				if(left>0){
					$("#spendAnalysis").text("同比上月，当前月的同期支出比上个月多了"+left+"元");
				}
				else if(left==0){
					$("#spendAnalysis").text("同比上月，当前月的同期支出与上个月持平");
				}
				else{
					$("#spendAnalysis").text("同比上月，当前月的同期支出比上个月少了"+-left+"元");
				}
//				
...
//				 alert("fail");
	...
	function currentTimeLine(data,currentTime){//当前月，当前时间收支图
		console.log("current.incomes:", data.current.incomes);
		console.log("length:", data.current.incomes.length);
		console.log(data.current.incomes.length==0);
//		alert("执行");
...
//			alert("你好："+data.current.incomes.length);
			...
			for(var i=0;i<data.current.incomes.length;i++){//当前月
				...
				console.log("current.incomes[i]:", data.current.incomes[i].categoryName);
			}
			for(var i=0;i<data.last.incomes.length;i++){//上个月
				arr.push(data.last.incomes[i].categoryName);
				console.log("last.incomes[i]:", data.current.incomes[i].categoryName);
...
//		alert(data.current.spends.length==0&&data.last.spends.length==0);
		console.log("current1.spends:", data.current.spends);
		console.log("length1:", data.current.spends.length);
		console.log(data.current.spends.length==0);
...
//		if(data.last==null){
			...
			for(var i=0;i<data.current.spends.length;i++){//当前月
				...
				console.log("current.spends[i]:", data.current.spends[i].categoryName);
			}
			for(var i=0;i<data.last.spends.length;i++){//上个月
				arr.push(data.last.spends[i].categoryName);
				console.log("current.spends[i]:", data.last.spends[i].moneyName);
			}
			$.unique(arr); //去重
//			alert(arr);
