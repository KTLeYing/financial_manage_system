...
//		alert(str);
		if(confirm('确认要批量删除该这些收支记录吗?')){
			$.post("/" + window.document.location.pathname.split("/")[1] + "/shouzhiRecord/deleteBatch.action", {"id":str}, function(data) {
			//回调为ok时，弹出alert框，并重新刷新页面
			alert("删除收支记录成功！");

			//checkbox全选，全不选的实现
			/*var arr = document.getElementsByName("record");
			for (var i = 0; i < arr.length; i++){
				arr[i].checked = false;
			}*/

			$('input[name="record"]').prop("checked", this.checked = false);
...
//			 $("#add_income_category_msg").text("");
			//ajax异步请求，当前收入类型是否存在
			 $.ajax({
					//请求资源路径
					url:"/" + window.document.location.pathname.split("/")[1] + "/shouzhiCategory/findsonCategoryByNameAndAjax.action",
					async:true,
					type:"post",
	 ...
	 $("#add_income_category_btn").click(function(){
		 ...
		 if(son!=null&&son!=""){
		 	...
		 	checkIncomeCategory();
		 	console.log("ssss");
		 	var lable = document.getElementById("add_income_category_msg");
		 	console.log("add_income_category_msg:", lable.innerText);
		 	if (lable.innerText==""){
				//提交表单数据
				$.post("/" + window.document.location.pathname.split("/")[1] + "/shouzhiCategory/addShouzhiCategory.action", $("#add_income_category_form")
...
//					$("#add_income_num_msg").text("请输入数字，不能带有字母或其它");
	...
	$("#add_income_szr_date").blur(function(){
...
//			 $("#add_spend_category_msg").text("");
	 ...
	 $("#add_spend_category_btn").click(function(){
		 ...
		 if(son!=null&&son!=""){
		 	//调用函数判断是否存在
			 checkSpendCategory();
			 var lable = document.getElementById("add_spend_category_msg");
			 console.log("add_spend_category_msg:", lable.innerText);
			 if(lable.innerText==""){
				 //提交表单数据
				 $.post("/" + window.document.location.pathname.split("/")[1] + "/shouzhiCategory/addShouzhiCategory.action", $("#add_spend_category_form")
					 .serialize(), function(data) { //序列化数据为对象
					 //回调为ok时，弹出alert框，并重新刷新页面
					 alert("添加支出类型成功！");
					 window.location.reload();
...
//			 $("#add_spend_num_msg").text("");
			 ...
			 if(!isNaN(num)){//是数字
				 $("#msgSpendLabel").text("");
					...
					$("#msgSpendLabel").text("请输入数字，不能带有字母或其它");
				}
		 }
	 });

	$("#add_spend_szr_date").blur(function(){
		var date=$("#add_spend_szr_date").val();
		if(date==null||date==""){
			$("#msgSpendLabel").text("支出日期不能为空");
		}
		else{
			$("#msgSpendLabel").text("");
		}
